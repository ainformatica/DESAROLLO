...
function TablaEstudiante() {
    table = $("#tablaestudiantes").DataTable({
      ...
      processing: true,
      ajax: {
        url: "../Controlador/tabla_estudiante_controlador.php",
        type: "POST",
      },
                  ...
                  row: _createNode(sheet, "is", {
                    children: {
                      row: _createNode(sheet, "t", {
                        text: "           DEPARTAMENTO DE INFORMÁTICA, UNAH - REPORTE DE ESTUDIANTES ",
                      }),
                    },
                  }),
                },
              ...
              return {
                columns: [
  ...
  
                  {
                    ...
                    text: [
                      "UNIVERSIDAD NACIONAL AUTÓNOMA DE HONDURAS                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           FACULTAD DE CIENCIAS ECONÓMICAS, ADMINISTRATIVAS Y CONTABLES                                                                                                                                                                                                                                    DEPARTAMENTO DE INFORMÁTICA ",
                    ],
                    bold: true,
                  },
    ...
    document.getElementById("tablaestudiantes_filter").style.display = "none";
  ...
  function filterGlobal() {
    $("#tablaestudiantes").DataTable().search($("#global_filter").val()).draw();
  }
  
  var num_uno = document.getElementById("txt_uno").value;
  var num_dos = document.getElementById("txt_dos").value;
  
  //funciones de activar usuario
  $("#tablaestudiantes").on("click", ".activar", function () {
    ...
    if (data.Estado == "ACTIVO") {
      ...
      swal("Alert", "El estudiante " + mensaje + "", "warning");
    } else {
      swal({
        title: "¡Alerta!",
        text: "¿Está seguro de activar el estudiante?",
        icon: "warning",
        buttons: true,
        dangerMode: false,
      }).then((willDelete) => {
  ...
  $("#tablaestudiantes").on("click", ".desactivar", function () {
    var data = table.row($(this).parents("tr")).data();
    if (table.row(this).child.isShown()) {
      var data = table.row(this).data();
    }
    if (data.Estado == "INACTIVO") {
      ...
      swal({
        title: "¡Alerta!",
        text: "¿Está seguro de desactivar el estudiante?",
        icon: "warning",
        buttons: true,
        dangerMode: false,
      }).then((willDelete) => {
  ...
  function Modificar_Estatus(id_persona_, Estado) {
    ...
    $.ajax({
      url: "../Controlador/actualizar_estado_estudiantes_controlador.php",
      type: "POST",
      data: {
      ...
      if (resp > 0) {
        swal(
          "¡Buen trabajo!",
          "El estudiante se " + mensaje + " con éxito",
          "success"
        );
        table.ajax.reload();
      } else {
  ...
  function llenar_selectNACI() {
    ...
    $.ajax({
      url: "../Controlador/gestion_estudiantes_controlador.php?op=selectNACI",
      type: "POST",
      data: cadena,
      success: function (r) {
        //  console.log(r);
  ...
  function llenar_selectCAR() {
    var cadena = "&activar=activar";
    $.ajax({
      url: "../Controlador/gestion_estudiantes_controlador.php?op=selectCAR",
      type: "POST",
      data: cadena,
      success: function (r) {
        //  console.log(r);
  
        $("#cb_carrera_edita").html(r).fadeIn();
      },
    });
  }
  llenar_selectCAR();
  ...
  function llenar_selectCR() {
    var cadena = "&activar=activar";
    $.ajax({
      url: "../Controlador/gestion_estudiantes_controlador.php?op=selectCR",
      type: "POST",
      data: cadena,
      success: function (r) {
        //  console.log(r);
  
        $("#cb_credita").html(r).fadeIn();
      },
    });
  }
  llenar_selectCR();
  ...
  function ValidarIdentidad(identidad) {
    ...
    if (n == 5) {
      var ver = false;
      $.post(
        "../Controlador/registro_estudiantes_controlador.php?op=validar_depto",
        { codigo: contar },
  ...
  function modificar_gestion_estudiante() {     
    var identidad = $("#identidad_edita").val();
    console.log(identidad);

    var ncuenta = $("#ncuenta_edita").val();
    console.log(ncuenta);
  
    
    var carrera_ = $("#cb_carrera_edita").children('option:selected').val();
    console.log(carrera_);

    var cregional_ = $("#cb_credita").children('option:selected').val();
    console.log(cregional_);

    var egresado = $("#egresado_edita").val();
    console.log(egresado);
    
    var id_persona1 = $("#txt_id_persona").val();
    console.log(id_persona1);

      $.ajax({
        url: "../Controlador/gestion_estudiantes_controlador.php?op=modificar_gestion_estudiante",
        type: "POST",
        data: {
          identidad: identidad,
          ncuenta: ncuenta, 
          carrera_: carrera_,          
          cregional_: cregional_,
          egresado: egresado,
        ...
        if (resp > 0) {
          ...
          document.getElementById("cb_carrera_edita").value = "";
          document.getElementById("cb_credita").value = "";
          document.getElementById("egresado_edita").value = "";
