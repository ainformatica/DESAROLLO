...
$sql2 = $mysqli->prepare("SELECT concat(p.nombres,' ', p.apellidos) as nombre ,px.valor n_cuenta, ep.nombre_empresa, ep.jefe_inmediato, ep.telefono_jefe_inmediato, 
        ct.valor correo,
              ...
              WHEN 4 THEN 'ADJUNTAR DOCUMENTO OFICIO'
        
                ELSE
                'PENDIENTE' END AS PROCESO,sd.estado_coordinacion
                FROM  tbl_personas p, tbl_subida_documentacion sd , tbl_personas_extendidas px, tbl_practica_estudiantes pe, tbl_empresas_practica ep,
                tbl_contactos ct
              WHERE p.id_persona=px.id_persona AND sd.id_persona=p.id_persona AND px.id_atributo=12 AND pe.id_modalidad != 3
              AND ct.id_tipo_contacto = 4 AND ct.id_persona = p.id_persona AND ep.id_persona=p.id_persona AND pe.id_persona=p.id_persona;");
        $sql2->execute();
        ...
        <div class="content-wrapper">
          ...
          <section class="content-header">
            <div class="container-fluid">
              <div class="row mb-2">
                <div class="col-sm-6">
                  <h1> Listado de Alumnos Modalidad Presencial y Semipresencial </h1>
                </div>
...
if ($row2['estado_coordinacion']<>'1' and $row2['estado_coordinacion']<>'0' ) {
                      ?>
                      <tr>
                        <td><?php echo $row2['nombre']; ?></td>
                        <td><?php echo $row2['n_cuenta']; ?></td>
                        <td><?php echo $row2['nombre_empresa']; ?></td>
                        <td><?php echo $row2['jefe_inmediato']; ?></td>
                        <td><?php echo $row2['telefono_jefe_inmediato']; ?></td>
                        <td><?php echo $row2['correo']; ?></td>
                        <td><?php echo $row2['PROCESO']; ?></td>
